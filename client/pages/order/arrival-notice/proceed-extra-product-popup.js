...
export class ProceedExtraProductPopup extends localize(i18next)(LitElement) {
  ...
  updated(changedProps) {
    if (changedProps.has('data')) {
      this.approvedData = {
        ...this.data,
        records: this.data.records.filter(item => item.status === ORDER_PRODUCT_STATUS.READY_TO_APPROVED.value)
      }
    }
  }
  ...
  async _proceedExtraProducts() {
    try {
      ...
      const response = await client.query({
        query: gql`
          mutation {
            proceedExtraProducts(${gqlBuilder.buildArgs({
              ganNo: this.ganNo,
...
window.customElements.define('proceed-extra-product-popup', ProceedExtraProductPopup)
