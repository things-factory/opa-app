...
class CreateTransportOrder extends connect(store)(localize(i18next)(PageView)) {
      ...
      if (
        ...
        if (!response.errors) {
          ...
          Swal.fire({
            // position: 'top-end',
            type: 'success',
            title: 'Collection order created',
            // showConfirmButton: false,
            timer: 1500
          })
    ...
    if (this.vasGrist.data.records && this.vasGrist.data.records.length) {
      ...
      if (vasBatches.filter((vasBatch, idx, vasBatches) => vasBatches.indexOf(vasBatch) !== idx).length)
        Swal.fire({
          // position: 'top-end',
          type: 'error',
          title: 'Duplicated vas on same batch',
          // showConfirmButton: false,
          timer: 1500
        })
  ...
  async fetchDeliveryOrder() {
    ...
    if (!response.errors) {
      ...
      this._orderStatus = response.data.deliveryOrder.status
      this.productData = {
        ...this.productData,
  ...
  async _editDeliveryOrder(deliveryOrder) {
    const response = await client.query({
      query: gql`
        mutation {
          editDeliveryOrder(${gqlBuilder.buildArgs({
            name: this._orderName,
            deliveryOrder
          })}) {
            name 
