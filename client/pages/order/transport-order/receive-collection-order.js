...
class ReceiveCollectionOrder extends localize(i18next)(PageView) {
      ...
      <div class="co-form-container">
        <form name="collectionOrder" class="multi-column-form">
          <fieldset>
            ...
            <input name="cargoType" placeholder="${i18next.t('label.bag_crates_carton_ibc_drums_pails')}" readonly />
  ...
  async _receiveCollectionOrder() {
    try {
      const result = await CustomAlert({
        ...
        text: i18next.t('text.receive_collection_order'),
      ...
      const response = await client.query({
        query: gql`
          mutation {
            receiveCollectionOrder(${gqlBuilder.buildArgs(args)}) {
              name
            }
          }
        `
