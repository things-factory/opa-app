...
class ReturnOrderDetail extends connect(store)(localize(i18next)(PageView)) {
  ...
  get context() {
    return {
      title: i18next.t('title.return_order_detail'),
  ...
  async _deleteReturnOrder() {
    try {
      ...
      const response = await client.query({
        query: gql`
            mutation {
              deleteReturnOrder(${gqlBuilder.buildArgs({
                name: this._returnOrderNo
              })})
            }
          `
      ...
      if (!response.errors) {
        this._showToast({ message: i18next.t('text.return_order_has_been_deleted') })
        navigate(`return_orders`)
      }
    } catch (e) {
      this._showToast(e)
    }
  ...
  async _submitCancellationReturnOrder() {
    try {
      ...
      const response = await client.query({
        query: gql`
            mutation {
              pendingCancellationReturnOrder(${gqlBuilder.buildArgs({
                name: this._returnOrderNo
              })})
            }
          `
      ...
      if (!response.errors) {
        this._showToast({ message: i18next.t('text.return_order_is_submitted_for_cancellation') })
        navigate(`return_orders`)
      }
    } catch (e) {
      throw e
  ...
  async _confirmReturnOrder() {
    let result = await CustomAlert({
      title: i18next.t('title.are_you_sure'),
      text: i18next.t('text.confirm_return_order'),
      confirmButton: { text: i18next.t('button.confirm') },
      cancelButton: { text: i18next.t('button.cancel') }
    })
    ...
    try {
      const response = await client.query({
        query: gql`
            mutation {
              confirmReturnOrder(${gqlBuilder.buildArgs({
                name: this._returnOrderNo
              })}) {
                name
              }
      ...
      if (!response.errors) {
        this._showToast({ message: i18next.t('text.return_order_confirmed') })
        navigate('return_orders')
      }
    } catch (e) {
      this._showToast(e)
...
window.customElements.define('return-order-detail', ReturnOrderDetail)
