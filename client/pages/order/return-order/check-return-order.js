...
class CheckReturnOrder extends connect(store)(localize(i18next)(PageView)) {
  ...
  get context() {
    return {
      title: i18next.t('title.check_return_order'),
      actions: [
        {
          title: i18next.t('button.arrived'),
          action: this._checkReturnOrder.bind(this)
        },
        {
  ...
  async _checkReturnOrder() {
    try {
      ...
      const response = await client.query({
        query: gql`
          mutation {
            checkReturnOrder(${gqlBuilder.buildArgs({
              name: this._returnOrderNo
            })}) {
              name
            }
      ...
      if (!response.errors) {
        navigate(`return_order_assign_buffer_location/${this._returnOrderNo}`)

        this._showToast({ message: i18next.t('text.return_order_is_arrived') })
      }
    } catch (e) {
      this._showToast(e)
    }
...
window.customElements.define('check-return-order', CheckReturnOrder)
