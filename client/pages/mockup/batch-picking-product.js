...
class BatchPickingProduct extends connect(store)(localize(i18next)(PageView)) {
  ...
  render() {
    return html`
      <form id="info-form" class="multi-column-form">
        ...
        <fieldset>
          <legend>${`${i18next.t('title.wave_no')}: ${this.releaseGoodNo}`}</legend>
            ...
            ${this.scannable
              ? html`
                  <fieldset>
                    ...
                    <label>${i18next.t('label.bin_no')}</label>
                    <barcode-scanable-input name="binNo" custom-input></barcode-scanable-input>
...
window.customElements.define('batch-picking-product', BatchPickingProduct)
